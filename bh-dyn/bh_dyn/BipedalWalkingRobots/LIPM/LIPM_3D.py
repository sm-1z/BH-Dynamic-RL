...
class LIPM3D:
    ...
    def initializeModel(self, COM_pos, left_foot_pos, right_foot_pos):
        ...
        self.zc = self.COM_pos[2] - self.left_foot_pos[2]
    ...
    def nextReferenceFootLocation(self, s_x, s_y, theta=0):
        if self.support_leg is 'left_leg': # then the next support leg is the right leg
            p_x_new = self.p_x + np.sin(theta)*s_x - np.cos(theta)*s_y #!
            p_y_new = self.p_y + np.cos(theta)*s_x + np.sin(theta)*s_y #!
    ...
    def nextState(self, s_x, s_y, theta=0):
        ...
        if self.support_leg is 'left_leg':
            bar_x_new = np.sin(theta)*s_x/2.0 - np.cos(theta)*s_y/2.0 #!
            bar_y_new = np.cos(theta)*s_x/2.0 + np.sin(theta)*s_y/2.0 #!
    ...
    def nextVel(self, bar_x=0, bar_y=0, theta=0):
        ...
        bar_vx_new = np.cos(theta)*(C-1)/(T_c*S)*bar_x - np.sin(theta)*(C+1)/(T_c*S)*bar_y
        bar_vy_new = np.sin(theta)*(C-1)/(T_c*S)*bar_x + np.cos(theta)*(C+1)/(T_c*S)*bar_y
