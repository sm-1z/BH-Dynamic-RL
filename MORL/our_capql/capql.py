...
class ReplayMemory:
    ...
    def sample(self, batch_size, to_tensor=True, device=None):
        ...
        if to_tensor:
            return tuple(
                map(
                    lambda x: th.tensor(x, dtype=th.float32).to(device),
                    experience_tuples,
                )
            )
...
class Policy(nn.Module):
    ...
    def __init__(
        self, obs_dim, rew_dim, output_dim, action_space, net_arch=[256, 256]
    ):
        ...
        self.register_buffer(
            "action_scale",
            th.tensor(
                (action_space.high - action_space.low) / 2.0, dtype=th.float32
            ),
        )
        self.register_buffer(
            "action_bias",
    ...
    def sample(self, obs, w):
        ...
        log_prob -= th.log(self.action_scale * (1 - y_t.pow(2)) + EPSILON).sum(
            dim=1
        )
...
class CAPQL(MOAgent, MOPolicy):
    ...
    def __init__(
        ...
        self.q_optim = optim.Adam(
            chain(*[net.parameters() for net in self.q_nets]),
            lr=self.learning_rate,
        )
        self.policy_optim = optim.Adam(
            list(self.policy.parameters()), lr=self.learning_rate
        )
        ...
        self._ep_info_buffer = deque(maxlen=100)
    ...
    def save(
        self, save_dir="weights/", filename=None, save_replay_buffer=True
    ):
        ...
        for i, (q_net, target_q_net) in enumerate(
            ...
            saved_params["q_net_" + str(i) + "_state_dict"] = (
                q_net.state_dict()
            )
            saved_params["target_q_net_" + str(i) + "_state_dict"] = (
    ...
    def update(self):
        ...
        for _ in range(self.gradient_updates):
            (s_obs, s_actions, w, s_rewards, s_next_obs, s_dones) = (
            ...
            with th.no_grad():
                ...
                q_targets = th.stack(
                    [
                        q_target(s_next_obs, next_actions, w)
                        for q_target in self.target_q_nets
                    ]
                )
                min_target_q = th.min(q_targets, dim=0)[
                    0
                ] - self.alpha * log_pi.reshape(-1, 1)

                target_q = (
                    s_rewards
                    + (1 - s_dones.reshape(-1, 1)) * self.gamma * min_target_q
                ).detach()
            ...
            q_values = th.stack(
                [q_target(s_obs, pi, w) for q_target in self.q_nets]
            )
            ...
            for q_net, target_q_net in zip(self.q_nets, self.target_q_nets):
                polyak_update(
                    q_net.parameters(), target_q_net.parameters(), self.tau
                )

        self.writer.add_scalar(
            "train/critic_loss", critic_loss.item(), self.global_step
        )
        self.writer.add_scalar(
            "train/actor_loss", policy_loss.item(), self.global_step
        )
        # self.writer.add_scalar('rollout/ep_raw_mean', safe_mean([ep_info_tmp["r"] for ep_info_tmp in self._ep_info_buffer]), self.global_step)
        # self.writer.add_scalar('rollout/ep_len_mean', safe_mean([ep_info_tmp["l"] for ep_info_tmp in self._ep_info_buffer]), self.global_step)

        if (
            self._ep_info_buffer is not None
            and len(self._ep_info_buffer) > 0
            and self._ep_info_buffer[0] is not None
        ):
            print(
                "---------_ep_info_buffer:",
                self._ep_info_buffer[0],
                "----------",
            )
            self._ep_info_buffer.popleft()
    ...
    def train(
        ...
        weight_sampler = WeightSamplerAngle(
            self.env.unwrapped.reward_dim, angle
        )
        ...
        path = os.path.join(
            self.logdir, self.train_num, "CAPQL_" + str(iteration)
        )
        self.writer = SummaryWriter(
            path,
            filename_suffix=str(iteration),
        )
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            next_obs, vector_reward, terminated, truncated, info = (
                self.env.step(action_env)
            )

            self.replay_buffer.push(
                obs, action, w, vector_reward, next_obs, terminated
            )
            ...
            if terminated or truncated:
                ...
                self.writer.add_scalar(
                    "rollout/ep_rew_mean", reward_sum, self.global_step
                )
                self.writer.add_scalar(
                    "rollout/ep_len_mean", self.current_step, self.global_step
                )

                self.writer.add_scalar(
                    "Dif_reward/forward_reward",
                    vec_reward[0],
                    self.global_step,
                )
                # self.writer.add_scalar('Dif_reward/healthy_reward', vec_reward[1], self.global_step)
                self.writer.add_scalar(
                    "Dif_reward/robustness_reward",
                    vec_reward[1],
                    self.global_step,
                )
                self.writer.add_scalar(
                    "Dif_reward/effic_reward", vec_reward[2], self.global_step
                )
                self.current_step = 0
                reward_sum = 0
                ...
                if self.log and "episode" in info.keys():
                    log_episode_info(
                        info["episode"], np.dot, w, self.global_step
                    )
                    ep_info = info.get("episode")
                    self._ep_info_buffer.extend([ep_info])
            ...
            if self.log and self.global_step % eval_freq == 0:
                ...
                returns_test_tasks = [
                    policy_evaluation_mo(
                        self, eval_env, ew, rep=num_eval_episodes_for_front
                    )[3]
            ...
            if checkpoints:
                self.save(
                    "./model/BH/", filename="CAPQL_BH", save_replay_buffer=True
                )

            # self.writer.add_scalar('train/',)
